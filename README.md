...
Serverless offline was corrupting binary files on requests. To avoid this we have commented the line where encoding is done. We are unaware of what purpose that line had. This fork is only meant to be used when testing lambdas in node.js that recieve images or any type of binary data that might be corrupted.
